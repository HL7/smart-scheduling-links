...
- **Slot Aggregator**: an API service offered by third parties, aggregating data from multiple Slot Publishers or from other healthcare provider APIs. Slot Aggregators otherwise act in a similar capacity to Slot Publishers but are not themselves authoritative sources.
...
### UX Considerations and Slot Validity
...
- Slot data discovered by a _Slot Discovery Client_ may not reflect current availability by the time a user reaches the _Provider Booking Portal_. Staleness between publication cycles is one source of invalidity, but others exist — for example, eligibility requirements, referral prerequisites, provider schedule changes, or concurrent bookings. _Slot Publishers_ SHOULD provide data with appropriate freshness (see [Cache-Control guidance](specification.html#performance-considerations)). _Slot Discovery Clients_ and _Slot Aggregators_ MAY enrich slot data with additional attributes to improve provider-and-slot matching for their users. Because the full range of validity considerations is broad and context-dependent, this specification intentionally leaves much of the handling to implementers — with the key backstop being that the _Provider Booking Portal_ is the closest system to the source of truth. Provider Booking Portals SHOULD handle slot invalidity gracefully, preserving as much of the user's original intent as possible (see [Graceful Handling of Slot Invalidity](specification.html#graceful-handling-of-slot-invalidity) in the specification).
...
| Ron Urwongse | Provider Data SME |
| Josh Mandel | Editor |
