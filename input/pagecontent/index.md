...
- Slot data discovered by a _Slot Discovery Client_ may not reflect current availability by the time a user reaches the _Provider Booking Portal_. Staleness between publication cycles is one source of invalidity, but others exist — for example, eligibility requirements, referral prerequisites, provider schedule changes, or concurrent bookings. Because the full range of validity considerations is broad and context-dependent, this specification intentionally leaves much of the handling to implementers — with the key backstop being that the _Provider Booking Portal_ is the closest system to the source of truth. The division of responsibility among publishers, clients, aggregators, and booking portals is set out in [Graceful Handling of Slot Invalidity](specification.html#graceful-handling-of-slot-invalidity) in the specification.
